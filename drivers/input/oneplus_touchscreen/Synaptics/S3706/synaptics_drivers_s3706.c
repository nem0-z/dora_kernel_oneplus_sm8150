...
static int tp_single_tap_en(struct chip_data_s3706 *ts, bool enable)
{
	...
	if (enable) {
		...
		//              0x1F, 0x01);
		ret = touch_i2c_write_byte(ts->client, 0x20, 0x1e);
		ret = touch_i2c_write_byte(ts->client, 0x21, 0x1e);
		ret = touch_i2c_write_byte(ts->client, 0x22, 0x32);
		//ret = touch_i2c_write_word(ts->client,
		//      F54_ANALOG_COMMAND_BASE, 0x04); // force update
	} else
		ret = touch_i2c_write_byte(ts->client, 0x1F, 0x00);
				      ...
				      struct point_info *points, int max_num)
{
	...
	obj_attention =
	    touch_i2c_read_word(chip_info->client,
				chip_info->reg_info.F12_2D_DATA15);
	for (i = 9;; i--) {
	...
	ret =
	    touch_i2c_read_block(chip_info->client,
				 chip_info->reg_info.F12_2D_DATA_BASE,
...
static int synaptics_get_vendor(void *chip_data, struct panel_info *panel_data)
{
	...
	strncpy(panel_data->manufacture_info.manufacture, manu_temp,
		MAX_DEVICE_MANU_LENGTH);
	TPD_INFO
	    ("chip_info->tp_type = %d, panel_data->test_limit_name = %s, panel_data->fw_name = %s\n",
	     chip_info->tp_type, panel_data->test_limit_name,
	     panel_data->fw_name);
	return 0;
}
...
static int synaptics_get_chip_info(void *chip_data)
{
	...
	TPD_INFO("F12_2D_QUERY_BASE = 0x%x \n\
			...
			F12_2D_DATA_BASE   = 0x%x \n", reg_info->F12_2D_QUERY_BASE, reg_info->F12_2D_CMD_BASE, reg_info->F12_2D_CTRL_BASE, reg_info->F12_2D_DATA_BASE);
	...
	TPD_INFO("F01_RMI_QUERY_BASE = 0x%x \n\
			...
			F01_RMI_DATA_BASE   = 0x%x \n", reg_info->F01_RMI_QUERY_BASE, reg_info->F01_RMI_CMD_BASE, reg_info->F01_RMI_CTRL_BASE, reg_info->F01_RMI_DATA_BASE);
	...
	TPD_INFO("F34_FLASH_QUERY_BASE   = 0x%x \n\
			...
			F34_FLASH_DATA_BASE         = 0x%x \n", reg_info->F34_FLASH_QUERY_BASE, reg_info->F34_FLASH_CMD_BASE, reg_info->F34_FLASH_CTRL_BASE, reg_info->F34_FLASH_DATA_BASE);
	...
	TPD_INFO("F51_CUSTOM_QUERY_BASE  = 0x%x \n\
			...
			F51_CUSTOM_DATA_BASE        = 0x%x \n", reg_info->F51_CUSTOM_QUERY_BASE, reg_info->F51_CUSTOM_CMD_BASE, reg_info->F51_CUSTOM_CTRL_BASE, reg_info->F51_CUSTOM_DATA_BASE);
...
static fw_check_state synaptics_fw_check(void *chip_data,
				      ...
				      bool enable)
{
	...
	ret =
	    touch_i2c_write_byte(chip_info->client,
				 chip_info->reg_info.F01_RMI_CTRL01,
				 abs_status_int);
	if (ret < 0) {
		TPD_INFO
		    ("%s: enable or disable abs interrupt failed, abs_int = %d\n",
...
static int synaptics_configuration_init(struct chip_data_s3706 *chip_info,
					bool config)
{
	int ret = 0;
	...
	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F01_RMI_CTRL00);
	if (ret < 0) {
					  ...
					  bool enable)
{
	...
	if (enable) {
		...
		ret = touch_i2c_write_byte(chip_info->client,
					   chip_info->reg_info.F01_RMI_CTRL02,
					   0x3);
	} else {
...
static int synaptics_enable_edge_limit(struct chip_data_s3706 *chip_info,
				       bool enable)
{
	int ret;
	...
	ret =
	    touch_i2c_read_byte(chip_info->client,
				chip_info->reg_info.F51_CUSTOM_CTRL50);
	if (enable) {
		ret |= 0x01;
...
static int synaptics_enable_face_detect(struct chip_data_s3706 *chip_info,
					bool enable)
{
	int ret = -1;
	int doze_status = 0;
	...
	if (enable) {
		...
		if (syna_tp->ctl_base_address) {
			ret |= touch_i2c_write_byte(chip_info->client,
						    chip_info->reg_info.
						    F51_CUSTOM_DATA_BASE + 0x33,
						    0x01);
		} else {
			ret |= touch_i2c_write_byte(chip_info->client,
						    chip_info->reg_info.
						    F51_CUSTOM_DATA_BASE + 0x38,
						    0x01);
		}
		ret |= touch_i2c_write_byte(chip_info->client, 0xff, 0x00);
	} else {
...
static void synaptics_enable_charge_mode(struct chip_data_s3706 *chip_info,
					 bool enable)
{
	int ret = 0, arg = 0;
	...
	if (enable) {
		...
		TPD_DEBUG("%s enable is %d, arg is 0x%x\n", __func__, enable,
			  arg);
		touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.F01_RMI_CTRL00, arg);
	} else {
		arg = ret & 0xDF;
		TPD_DEBUG("%s enable is %d, arg is 0x%x\n", __func__, enable,
			  arg);
				       ...
				       bool enable)
{
	...
	ret =
	    touch_i2c_write_block(chip_info->client,
				  chip_info->reg_info.F12_2D_CTRL11, 13,
				  &game_buffer[0]);
	if (ret < 0) {
...
static void synaptics_touchhold(struct chip_data_s3706 *chip_info, bool enable)
{
	...
	if (enable) {
		...
		if (syna_tp->ctl_base_address && !syna_tp->tx_change_order) {
			...
			ret =
			    touch_i2c_write_byte(chip_info->client, 0x29, ret);
		} else {
			ret = touch_i2c_read_byte(chip_info->client, 0x29);
			ret &= 0xFE;
			chip_info->en_up_down = 0;
				      ...
				      struct gesture_info *gesture)
{
	...
	/*get gesture type */
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.F12_2D_DATA04, 5,
				   &(gesture_buffer[0]));
	...
	TPD_DETAIL
	    ("%s, gesture_sign = 0x%x, gesture_type = %d regswipe is %d\n",
	...
	if (gesture->gesture_type != UnkownGesture) {
		for (i = 0; i < 23; i += 2) {
			...
			TPD_DEBUG
			    ("synaptics TP read coordinate_point[%d] = %d\n", i,
			     trspoint);
		}
		...
		gesture->Point_start.x =
		    (coordinate_buf[0] | (coordinate_buf[1] << 8));
		gesture->Point_start.y =
		    (coordinate_buf[2] | (coordinate_buf[3] << 8));
		gesture->Point_end.x =
		    (coordinate_buf[4] | (coordinate_buf[5] << 8));
		gesture->Point_end.y =
		    (coordinate_buf[6] | (coordinate_buf[7] << 8));
		gesture->Point_1st.x =
		    (coordinate_buf[8] | (coordinate_buf[9] << 8));
		gesture->Point_1st.y =
		    (coordinate_buf[10] | (coordinate_buf[11] << 8));
		gesture->Point_2nd.x =
		    (coordinate_buf[12] | (coordinate_buf[13] << 8));
		gesture->Point_2nd.y =
		    (coordinate_buf[14] | (coordinate_buf[15] << 8));
		gesture->Point_3rd.x =
		    (coordinate_buf[16] | (coordinate_buf[17] << 8));
		gesture->Point_3rd.y =
		    (coordinate_buf[18] | (coordinate_buf[19] << 8));
		gesture->Point_4th.x =
		    (coordinate_buf[20] | (coordinate_buf[21] << 8));
		gesture->Point_4th.y =
...
static int fwu_write_f34_v7_partition_id(struct chip_data_s3706 *chip_info,
					 unsigned char cmd)
{
	int retval;
	unsigned char data_base;
	...
	retval = touch_i2c_write_block(chip_info->client,
				       data_base +
				       chip_info->fwu->off.partition_id,
...
static int fwu_write_f34_partition_id(struct chip_data_s3706 *chip_info,
				      unsigned char cmd)
{
	int retval = -1;
...
static int fwu_read_flash_status(struct chip_data_s3706 *chip_info)
{
	...
	if (chip_info->fwu->flash_status != 0x00) {
		TPD_INFO("%s: Flash status = %d, command = 0x%02x\n",
			 __func__, chip_info->fwu->flash_status,
			 chip_info->fwu->command);
	}
...
static int fwu_wait_for_idle(struct chip_data_s3706 *chip_info, int timeout_ms,
			     bool poll)
{
	int count = 0;
	...
	do {
		...
		if ((chip_info->fwu->command == CMD_IDLE)
		    && (chip_info->fwu->flash_status == 0x00)) {
			return 0;
		}
...
static int fwu_write_f34_v7_command_single_transaction(struct chip_data_s3706
						       *chip_info,
						       unsigned char cmd)
{
	int retval;
...
static int fwu_write_f34_v7_command(struct chip_data_s3706 *chip_info,
				    unsigned char cmd)
{
	int retval;
	unsigned char data_base;
	...
	case CMD_ENABLE_FLASH_PROG:
		retval =
		    fwu_write_f34_v7_command_single_transaction(chip_info, cmd);
		if (retval < 0) {
			return retval;
		} else {
			return 0;
...
static int fwu_write_f34_command(struct chip_data_s3706 *chip_info,
				 unsigned char cmd)
{
	int retval = -1;
...
static int fwu_read_f34_v7_partition_table(struct chip_data_s3706 *chip_info,
					   unsigned char *partition_table)
{
	...
	retval = touch_i2c_write_block(chip_info->client,
				       data_base +
				       chip_info->fwu->off.block_number,
				       sizeof(block_number),
	...
	length[0] =
	    (unsigned char)(chip_info->fwu->flash_config_length & MASK_8BIT);
	length[1] = (unsigned char)(chip_info->fwu->flash_config_length >> 8);

	retval = touch_i2c_write_block(chip_info->client,
				       data_base +
				       chip_info->fwu->off.transfer_length,
...
static void fwu_parse_partition_table(struct chip_data_s3706 *chip_info,
...
static int fwu_read_f34_queries(struct chip_data_s3706 *chip_info)
{
	...
	memset(&chip_info->fwu->blkcount, 0x00,
	       sizeof(chip_info->fwu->blkcount));
...
static int synaptics_rmi4_fwu_init(struct chip_data_s3706 *chip_info,
				   bool force)
{
	int retval;
	struct pdt_properties pdt_props = { 0 };
...
static void fwu_parse_image_header_10_bootloader(struct chip_data_s3706
						 *chip_info,
						 const unsigned char *image)
{
	...
	for (ii = 1; ii <= num_of_containers; ii++) {
		addr =
		    le_to_uint(chip_info->fwu->img.bootloader.data + (ii * 4));
...
static void fwu_parse_image_header_10_utility(struct chip_data_s3706 *chip_info,
					      const unsigned char *image)
{
	unsigned char ii;
	unsigned char num_of_containers;
...
static void fwu_parse_image_header_10(struct chip_data_s3706 *chip_info)
{
	...
	for (ii = 0; ii < num_of_containers; ii++) {
		...
		case GENERAL_INFORMATION_CONTAINER:
			...
			chip_info->fwu->img.firmware_id =
			    le_to_uint(content + 4);
			break;
		default:
			break;
		}
...
static void fwu_compare_partition_tables(struct chip_data_s3706 *chip_info)
{
	...
	if (chip_info->fwu->phyaddr.bl_image !=
	    chip_info->fwu->img.phyaddr.bl_image) {
		chip_info->fwu->incompatible_partition_tables = true;
	} else if (chip_info->fwu->phyaddr.lockdown !=
		   chip_info->fwu->img.phyaddr.lockdown) {
		chip_info->fwu->incompatible_partition_tables = true;
	} else if (chip_info->fwu->phyaddr.bl_config !=
		   chip_info->fwu->img.phyaddr.bl_config) {
		chip_info->fwu->incompatible_partition_tables = true;
	} else if (chip_info->fwu->phyaddr.utility_param !=
		   chip_info->fwu->img.phyaddr.utility_param) {
		chip_info->fwu->incompatible_partition_tables = true;
	}

	if (chip_info->fwu->bl_version == BL_V7) {
		if (chip_info->fwu->phyaddr.fl_config !=
		    chip_info->fwu->img.phyaddr.fl_config) {
			chip_info->fwu->incompatible_partition_tables = true;
		}
	}
	...
	if (chip_info->fwu->phyaddr.ui_firmware !=
	    chip_info->fwu->img.phyaddr.ui_firmware) {
		chip_info->fwu->new_partition_table = true;
	} else if (chip_info->fwu->phyaddr.ui_config !=
		   chip_info->fwu->img.phyaddr.ui_config) {
		chip_info->fwu->new_partition_table = true;
	}

	if (chip_info->fwu->flash_properties.has_disp_config) {
		if (chip_info->fwu->phyaddr.dp_config !=
		    chip_info->fwu->img.phyaddr.dp_config) {
			chip_info->fwu->new_partition_table = true;
		}
	}

	if (chip_info->fwu->has_guest_code) {
		if (chip_info->fwu->phyaddr.guest_code !=
		    chip_info->fwu->img.phyaddr.guest_code) {
			chip_info->fwu->new_partition_table = true;
		}
	}
...
static int fwu_parse_image_info(struct chip_data_s3706 *chip_info)
{
	...
	if (chip_info->fwu->bl_version == BL_V7
		...
		fwu_parse_partition_table(chip_info, chip_info->fwu->img.fl_config.data, &chip_info->fwu->img.blkcount, &chip_info->fwu->img.phyaddr);	//Roland firmware img in emmc
...
static int fwu_get_image_firmware_id(struct chip_data_s3706 *chip_info,
				     unsigned int *fw_id)
{
	int retval;
	...
	if (chip_info->fwu->img.contains_firmware_id) {
		*fw_id = chip_info->fwu->img.firmware_id;
		...
		strptr =
		    strnstr(chip_info->fwu->image_name, "PR",
			    MAX_IMAGE_NAME_LEN);
		if (!strptr) {
			TPD_INFO
...
static enum flash_area fwu_go_nogo(struct chip_data_s3706 *chip_info)
{
	...
	for (ii = 0; ii < config_id_size; ii++) {
		sprintf(tmp, "%02x",
			(unsigned char)chip_info->fwu->img.ui_config.data[ii]);
		memcpy(&config_id_img[ii * 2], tmp, 2);
		sprintf(tmp, "%02x",
			(unsigned char)chip_info->fwu->config_id[ii]);
		memcpy(&config_id_panel[ii * 2], tmp, 2);
		if (chip_info->fwu->img.ui_config.data[ii] !=
		    chip_info->fwu->config_id[ii]) {
...
static int fwu_write_f34_v7_blocks(struct chip_data_s3706 *chip_info,
...
static int fwu_write_f34_blocks(struct chip_data_s3706 *chip_info,
				unsigned char *block_ptr,
				unsigned short block_cnt, unsigned char cmd)
{
	...
	if (chip_info->fwu->bl_version == BL_V7
	    || chip_info->fwu->bl_version == BL_V8) {
		retval =
		    fwu_write_f34_v7_blocks(chip_info, block_ptr, block_cnt,
					    cmd);
	} else {
...
static int fwu_write_lockdown(struct chip_data_s3706 *chip_info)
{
	...
	lockdown_block_count =
	    chip_info->fwu->img.lockdown.size / chip_info->fwu->block_size;

	return fwu_write_f34_blocks(chip_info,
				    (unsigned char *)chip_info->fwu->img.
				    lockdown.data, lockdown_block_count,
				    CMD_WRITE_LOCKDOWN);
}
...
static int fwu_check_ui_configuration_size(struct chip_data_s3706 *chip_info)
{
	...
	block_count =
	    chip_info->fwu->img.ui_config.size / chip_info->fwu->block_size;
...
static int fwu_erase_configuration(struct chip_data_s3706 *chip_info)
{
	...
	case DP_CONFIG_AREA:
		retval =
		    fwu_write_f34_command(chip_info, CMD_ERASE_DISP_CONFIG);
		if (retval < 0)
			return retval;
		break;
	case BL_CONFIG_AREA:
	...
	case FLASH_CONFIG_AREA:
		retval =
		    fwu_write_f34_command(chip_info, CMD_ERASE_FLASH_CONFIG);
		if (retval < 0)
			return retval;
		break;
	case UPP_AREA:
		retval =
		    fwu_write_f34_command(chip_info,
...
{
	return fwu_write_f34_blocks(chip_info,
				    (unsigned char *)chip_info->fwu->
				    config_data,
				    chip_info->fwu->config_block_count,
				    CMD_WRITE_CONFIG);
}
...
static int fwu_check_ui_firmware_size(struct chip_data_s3706 *chip_info)
{
	...
	block_count =
	    chip_info->fwu->img.ui_firmware.size / chip_info->fwu->block_size;
...
static int fwu_check_dp_configuration_size(struct chip_data_s3706 *chip_info)
{
	...
	block_count =
	    chip_info->fwu->img.dp_config.size / chip_info->fwu->block_size;
...
static int fwu_check_guest_code_size(struct chip_data_s3706 *chip_info)
{
	...
	block_count =
	    chip_info->fwu->img.guest_code.size / chip_info->fwu->block_size;
...
static int fwu_check_bl_configuration_size(struct chip_data_s3706 *chip_info)
{
	...
	block_count =
	    chip_info->fwu->img.bl_config.size / chip_info->fwu->block_size;
...
static int fwu_erase_all(struct chip_data_s3706 *chip_info)
{
	...
	if (chip_info->fwu->bl_version == BL_V7) {
		retval =
		    fwu_write_f34_command(chip_info, CMD_ERASE_UI_FIRMWARE);
		if (retval < 0)
			return retval;
...
static int fwu_write_bootloader(struct chip_data_s3706 *chip_info)
{
	...
	bootloader_block_count =
	    chip_info->fwu->img.bl_image.size / chip_info->fwu->block_size;

	chip_info->fwu->write_bootloader = true;
	retval =
	    fwu_write_f34_blocks(chip_info,
				 (unsigned char *)chip_info->fwu->img.bl_image.
				 data, bootloader_block_count,
...
static int fwu_write_utility_parameter(struct chip_data_s3706 *chip_info)
{
 ...
 image_param:
		...
		retval = secure_memcpy(pbuf,
				       remaining_size,
				       chip_info->fwu->img.utility_param[ii].
				       data,
				       chip_info->fwu->img.utility_param[ii].
				       size,
				       chip_info->fwu->img.utility_param[ii].
	...
	chip_info->fwu->read_config_buf[utility_param_size - 4] =
	    checksum_array[0];
	chip_info->fwu->read_config_buf[utility_param_size - 3] =
	    checksum_array[1];
	chip_info->fwu->read_config_buf[utility_param_size - 2] =
	    checksum_array[2];
	chip_info->fwu->read_config_buf[utility_param_size - 1] =
	    checksum_array[3];
...
static int fwu_read_f34_v7_blocks(struct chip_data_s3706 *chip_info,
				  unsigned short block_cnt,
				  unsigned char command)
{
	int retval;
	...
	do {
		...
		retval = touch_i2c_read_block(chip_info->client,
					      data_base +
					      chip_info->fwu->off.payload,
					      transfer *
					      chip_info->fwu->block_size,
					      &chip_info->fwu->
					      read_config_buf[index]);
		if (retval < 0) {
			TPD_INFO
...
static int fwu_read_f34_blocks(struct chip_data_s3706 *chip_info,
			       unsigned short block_cnt, unsigned char cmd)
{
	int retval = 0;
...
static int fwu_write_flash_configuration(struct chip_data_s3706 *chip_info)
{
	...
	if (chip_info->fwu->config_block_count !=
	    chip_info->fwu->blkcount.fl_config) {
...
static int fwu_write_guest_code(struct chip_data_s3706 *chip_info)
{
	...
	retval =
	    fwu_write_f34_blocks(chip_info,
				 (unsigned char *)chip_info->fwu->img.
				 guest_code.data, guest_code_block_count,
...
static int fwu_write_firmware(struct chip_data_s3706 *chip_info)
{
	...
	return fwu_write_f34_blocks(chip_info,
				    (unsigned char *)chip_info->fwu->img.
				    ui_firmware.data, firmware_block_count,
				    CMD_WRITE_FW);
}
...
static int fwu_do_reflash(struct chip_data_s3706 *chip_info)
{
	...
	if (!chip_info->fwu->new_partition_table) {
		...
		if (chip_info->fwu->has_guest_code
		    && chip_info->fwu->img.contains_guest_code) {
	...
	if (!chip_info->fwu->has_utility_param
	    && chip_info->fwu->img.contains_utility_param) {
		if (chip_info->fwu->bl_version == BL_V7
		    || chip_info->fwu->bl_version == BL_V8)
			do_bl_update = true;
	}
	...
	if (do_bl_update) {
		...
		TPD_INFO("%s: Bootloader area programmed\n", __func__);
		   ...
		   && chip_info->fwu->new_partition_table) {
...
static int fwu_start_reflash(struct chip_data_s3706 *chip_info)
{
	...
	if (chip_info->fwu->blkcount.total_count !=
	    chip_info->fwu->img.blkcount.total_count) {
	...
	if (flash_area != NONE) {
		...
		if (retval < 0) {
			chip_info->fwu->rmi4_data->reset_device(rmi4_data,
								false);
			goto exit;
		}
	}
...
static fw_update_state synaptics_fw_update(void *chip_data,
...
static void synaptics_register_info_read(void *chip_data,
					 ...
					 uint8_t * result, uint8_t length)
{
	...
	touch_i2c_write_byte(chip_info->client, 0xff, h_tmp);	/*set page */
	touch_i2c_read_block(chip_info->client, l_tmp, length, result);	/*read data */
}
...
static int synaptics_tp_probe(struct i2c_client *client,
