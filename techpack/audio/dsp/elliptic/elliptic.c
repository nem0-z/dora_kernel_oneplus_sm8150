...
{	
	...
	if (ps_input_dev != NULL) {
		if (near) {
			input_event(ps_input_dev, EV_MSC, MSC_RAW, 1);
			input_sync(ps_input_dev);
		} else {
			input_event(ps_input_dev, EV_MSC, MSC_RAW, 0);
			input_sync(ps_input_dev);
		} 
	}
...
static const struct file_operations proximity_event_num_fops = {
    .read  = proximity_event_num_read,
    .open  = simple_open,
    .owner = THIS_MODULE,
};
...
int elliptic_driver_init(void)
{
    ...
    ps_input_dev->name = "elliptic_ultrasound_proximity";
    set_bit(EV_MSC, ps_input_dev->evbit);
    set_bit(MSC_RAW, ps_input_dev->mscbit);
	if (input_register_device(ps_input_dev)) {
            EL_PRINT_E("Failed to register ps input device");
            input_free_device(ps_input_dev);
            goto fail;
    }

	prEntry_tp = proc_mkdir("ultrasound", NULL);
	if( prEntry_tp == NULL ){
		EL_PRINT_E("Couldn't create ultrasound directory\n");
		goto fail;
	}
	prEntry_tmp = proc_create("event_num", 0666,prEntry_tp, &proximity_event_num_fops);
    if (prEntry_tmp == NULL) {
